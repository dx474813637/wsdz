<template>
	<view class="w" :style="{
		backgroundImage: `url(https://wx.rawmex.cn/Public/2023fenxiao/daren-06.png?time=${new Date().getTime()})`
	}"> 
		<view class="header-w u-p-20"> 
			<view class="  u-radius-10 u-p-10" style="background-color: #edf6ff;">
				...
				<view class="list bg-white u-radius-10 u-p-20 u-p-t-40 u-p-b-40 u-flex-column u-flex-items-center"> 
					 <view class="text-base">佣金收入(元)</view>
					 <view>
						 <u-count-to 
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() { 
				const res = await this.$api.fx_money()
				if(res.code == 1) {
