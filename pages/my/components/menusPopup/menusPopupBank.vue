							...
							<template v-if="keyword">
								<template v-if="searchRes.length > 0">
									<view class="searchRes-content u-p-20 u-flex u-flex-wrap u-flex-items-center">
										<view class="card-label rows u-line-1 u-m-b-12"
...
<script>
	...
	export default {
		...
		methods: {
			...
			...mapActions({ 
				ESIGN_GET_BANK_CODE: 'esign/ESIGN_GET_BANK_CODE', 
			}), 
			searchMenus() {
				this.searchRes = []
				let k = this.keyword.toUpperCase()
				let str = '' 
				this.bank.forEach(ele => { 
			...
			async getData() {
				this.loading = true
				await this.ESIGN_GET_BANK_CODE()
				this.loading = false; 
			},
			close() {
				this.$emit('close') 
