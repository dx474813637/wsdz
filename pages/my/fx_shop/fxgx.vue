<template>
	<view class="u-p-20" >
		...
		<view class="main u-p-30" v-if="list.id">
			...
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
				<view class="item text-light item-label">达人昵称</view>
				<view class="item u-text-right">{{list.da_company_fx.name}}</view>
			</view>  
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
				<view class="item text-light item-label">达人公司名</view>
				<view class="item u-text-right">{{list.da_company_fx.contact}}</view>
			</view>  
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
				<view class="item text-light item-label">达人电话</view>
				<view class="item u-text-right">{{list.da_company_fx.mobile}}</view>
			</view>  
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
			...
			<!-- <view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
				<view class="item text-light item-label">商品区域</view>
				<view class="item u-text-right">{{list.da_product.mobile}}</view>
			</view>  -->
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
				<view class="item text-light item-label">状态</view>
				<view class="item u-text-right" :class="{'text-primary': list.state == 1}">
					<text class="u-m-r-30">{{list.state == 1? '激活' : '未激活'}}</text>
				</view>
			</view>  
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
				<view class="item text-light item-label">店铺审核状态</view>
				<view class="item u-text-right u-flex u-flex-items-start" :class="{'text-primary': list.auth_state == 1}">
					<text class="u-m-r-30">{{list.auth_state == 1? '激活' : '未激活'}}</text> 
					<u-switch v-model="list.auth_state" activeValue="1" inactiveValue="0" asyncChange @change="asyncChange" ></u-switch>
				</view>
			</view>  
			<view class="main-row u-m-b-30 u-flex u-flex-items-start u-flex-between">
					...
					success: async (r) => {
						if (r.confirm) {
							...
							const res = await this.$api.shop_fxgx_change_status({
								params: { id: this.list.id, sign: this.list.sign }
							})
							if(res.code == 1) {
								uni.showToast({
									icon: 'none',
								...
								this.list.auth_state = 1 - this.list.auth_state + ''
